...
namespace RioSulAPI.Controllers
{
	public class ClienteController : ApiController
	{
		...
		/// <param name="Filtro"></param>
		/// <returns></returns>
		[System.Web.Http.Route("api/Cliente/Marcas")]
		[System.Web.Http.HttpGet]
		[ApiExplorerSettings(IgnoreApi = false)]
		public MARCA Marcas(string clave = "", string descripcion = "")
		{
			MARCA API = new MARCA();
			API.Marcas = new List<REQ_MARCA>();
			...
			try
			{
				...
				{
					_Conn.Open();
					string Consulta = @" select ClassID, Descr from ProductClass where dfltinvttype = 'F' and ClassID like '%"+ clave +"%' and Descr like '%"+ descripcion +"%' ;";
					SqlCommand Command = new SqlCommand(Consulta, _Conn);
					SqlDataReader reader = Command.ExecuteReader();
					while (reader.Read())
					{
						REQ_MARCA M = new REQ_MARCA();
						M.Clave = reader[0].ToString().Trim();
						M.Descripcion = reader[1].ToString().Trim();
		...
		/// <param name="Filtro"></param>
		/// <returns></returns>
		[System.Web.Http.Route("api/Cliente/ClienteMarcas")]
		[System.Web.Http.HttpPost]
		[ApiExplorerSettings(IgnoreApi = false)]
		public MESSAGE ClienteMarca([FromBody] REQ_CLIENTE_MARCA CM)
		{
			...
			try
			{
				...
				{
					foreach (REQ_MARCA item in CM.Marcas)
					{
						Models.C_ClienteMarca marca = new C_ClienteMarca();
						marca.id_Cliente = CM.IdClienteRef;
						marca.Clave = item.Clave;
						marca.Descripcion = item.Descripcion;

						db.C_ClienteMarca.Add(marca);
						db.SaveChanges();
					}
				}
		...
		[ApiExplorerSettings(IgnoreApi = false)]
		public MESSAGE updateClienteMarca([FromBody] REQ_CLIENTE_MARCA CM)
		{
			MESSAGE API = new MESSAGE();
			...
			try
			{
				using (Models.bd_calidadIIEntities db = new Models.bd_calidadIIEntities())
				{
					List<C_ClienteMarca> clienteMarca = db.C_ClienteMarca.Where(x => x.id_Cliente == CM.IdClienteRef).ToList();
					db.C_ClienteMarca.RemoveRange(clienteMarca);
		...
		[System.Web.Http.HttpGet]
		[ApiExplorerSettings(IgnoreApi = false)]
		public GET_CLIENTE_MARCA getClienteMarca(int idCliente)
		{
			GET_CLIENTE_MARCA API = new GET_CLIENTE_MARCA();
			API.Marcas = new List<REQ_MARCA>();
			...
			try
			{
				using (Models.bd_calidadIIEntities db = new Models.bd_calidadIIEntities())
				{
					Models.C_Clientes cliente = db.C_Clientes.Where(x => x.IdClienteRef == idCliente).FirstOrDefault();
					if (cliente != null)
					{
						API.IdClienteRef = idCliente;
						API.Descripcion = cliente.Descripcion;
						API.Observaciones = cliente.Observaciones;

						foreach (Models.C_ClienteMarca item in db.C_ClienteMarca.Where(x=> x.id_Cliente == idCliente).ToList())
						{
							REQ_MARCA marca = new REQ_MARCA();
							marca.Clave = item.Clave;
							marca.Descripcion = item.Descripcion;

							API.Marcas.Add(marca);
						}
					}
				}
				API.Message = new HttpResponseMessage(HttpStatusCode.OK);
		...
		public partial class REQ_MARCA
		{
		...
		{
			public List<REQ_MARCA> Marcas { get; set; }
			public HttpResponseMessage Message { get; set; }
		}

		public partial class REQ_CLIENTE_MARCA
		{
		...
		public partial class GET_CLIENTE_MARCA
		{
			public int IdClienteRef { get; set; }
			public string Descripcion { get; set; }
